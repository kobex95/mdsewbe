...
if [ $OSNAME != "macos" ];then
	...
	if [ ! -d /www/server/mdserver-web ];then
		curl --insecure -sSLo /tmp/master.tar.gz ${HTTP_PREFIX}github.com/kobex95/mdsewbe/archive/refs/heads/master.tar.gz
		cd /tmp && tar -zxvf /tmp/master.tar.gz
		mv -f /tmp/mdsewbe-master /www/server/mdserver-web
...
if [ "${OSNAME}" == "macos" ];then
	curl --insecure -fsSL ${HTTP_PREFIX}raw.githubusercontent.com/kobex95/mdsewbe/refs/heads/master/scripts/install/macos.sh | bash
else
